def blank_seating_chart(number_of_rows, seats_per_row)
  ...
  Array.new(number_of_rows).map { Array.new(seats_per_row) }
end
...
def count_empty_seats(chart)
  ...
  chart.reduce(0) do |sum, nils|
    sum + nils.count { |element| element == nil }
  end
...
def find_favorite(array_of_hash_objects)
  ...
  array_of_hash_objects.reduce(Hash.new(nil)) do |result, next_value|
    if next_value[:is_my_favorite?]  == true
